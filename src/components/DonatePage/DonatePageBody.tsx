...
const DonatePageBody: React.FC<DonatePageBodyProps> = ({ setIsConfirming }) => {
  ...
  const handleDonateClick = () => {
    ...
    if (parseFloat(inputAmount) < 20 || isNaN(parseFloat(inputAmount))) {
      console.log('The minimum donation amount is 20.');
      return;
    }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (regex.test(value)) {
      ...
      if (inputAmount < 20) {
        setInputErrorMessage('Minimum contribution: 20 POL');
      }
